  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-lg">
        <div className="flex items-center gap-3 flex-wrap mb-md">
            ...
            {STATUSES.map((s) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm border transition capitalize ${
                  status === s ? "bg-primary-container text-white border-primary-container" : "border-outline-variant text-on-surface-variant hover:border-primary-container"
                }`}
              >
                {s || "All"}
              </button>
          ...
          <table className="w-full text-left zebra-table">
            <thead>
              <tr className="bg-surface-container-low border-b border-outline-variant">
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Reference</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Guest</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Hotel</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Check-in</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Nights</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Amount</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant">Status</th>
                <th className="px-lg py-4 font-label-bold text-label-bold text-on-surface-variant text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="text-center py-16">
                    <div className="w-6 h-6 border-4 border-secondary-container border-t-transparent rounded-full animate-spin mx-auto" />
                  </td>
                </tr>
              ) : bookings.length === 0 ? (
                <tr>
                ...
                bookings.map((b) => (
                  <tr key={b.id}>
                    <td className="px-lg py-4 text-xs text-primary">{b.reference ?? `#${b.id}`}</td>
                    <td className="px-lg py-4">
                      <p className="font-medium text-primary text-sm">{b.user?.name ?? b.guest_name ?? "—"}</p>
                    ...
                    <td className="px-lg py-4 text-on-surface-variant text-sm">{fmt(b.check_in)}</td>
                    <td className="px-lg py-4 text-on-surface-variant text-sm">{b.nights ?? "—"}</td>
                    <td className="px-lg py-4 font-medium text-primary text-sm">AED {Number(b.total_price ?? 0).toLocaleString()}</td>
                    <td className="px-lg py-4">
                    ...
                    <td className="px-lg py-4 text-right">
                      <button onClick={() => openDetail(b)} className="text-primary-container hover:text-secondary transition">
        ...
        {(meta.last_page ?? 0) > 1 && (
          <div className="flex items-center justify-center gap-2 mt-md">
            {Array.from({ length: meta.last_page ?? 0 }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                onClick={() => setPage(p)}
                className={`w-9 h-9 rounded-lg text-sm transition ${p === page ? "bg-primary-container text-white" : "bg-white border border-outline-variant text-on-surface-variant hover:border-primary-container"}`}
              >
                {p}
              </button>
            ))}
                ...
                <DetailSection title="Update Status">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="bg-primary-container text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-40"
                    >
