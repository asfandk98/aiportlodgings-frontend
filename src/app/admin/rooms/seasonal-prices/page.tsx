      ...
      <div className="flex-1 overflow-y-auto p-lg">
        <div className="space-y-md max-w-3xl">
          ...
          {selectedRoom && (
            <>
              {selectedRoomData && (
                <div className="bg-white border border-outline-variant rounded-xl px-lg py-3 flex items-center justify-between">
                  <div>
                  ...
                  <button onClick={() => setShowForm((p) => !p)} className="flex items-center gap-2 bg-primary-container text-white px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition">
                ...
                <div className="bg-white border border-secondary-container rounded-xl p-lg space-y-md">
                  ...
                  <div className="grid grid-cols-3 gap-md">
                    <div className="space-y-1.5">
                      ...
                      <input type="number" min={0} value={form.price} onChange={(e) => setForm((p) => ({ ...p, price: e.target.value }))} placeholder="e.g. 700" className="admin-input" />
                    </div>
                    <div className="space-y-1.5">
                  ...
                  <div className="flex gap-3">
                    <button onClick={handleSave} disabled={saving} className="bg-primary-container text-white px-5 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-50">
                      {saving ? "Saving…" : "Save"}
                    </button>
                    <button
                      onClick={() => {
                  ...
                  <table className="w-full text-sm zebra-table">
                    <thead>
                      <tr className="border-b border-outline-variant text-on-surface-variant text-xs uppercase tracking-wider">
                        <th className="text-left px-lg py-3">Period</th>
                        <th className="text-left px-lg py-3">Seasonal Price</th>
                        <th className="text-left px-lg py-3">vs Base</th>
                        <th className="text-right px-lg py-3">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {prices.map((p) => {
                        ...
                        return (
                          <tr key={p.id}>
                            ...
                            <td className="px-lg py-4 text-primary font-semibold">
                              AED {p.price}
