...
const ROOM_AMENITIES = [
  "King Size Bed", "Twin Beds", "Blackout Curtains", "Soundproofing", "Airport View",
  "Mini Bar", "Work Desk", "Free WiFi", "Air Conditioning", "Smart TV",
];
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-lg">
        <div className="max-w-3xl space-y-md pb-xl">
          <form onSubmit={handleSubmit} className="space-y-md">
            ...
            <Section title="Room Details">
              <div className="grid grid-cols-2 gap-md">
                <Field label="Room Name *">
                  <input name="name" value={form.name} onChange={handleChange} required placeholder="e.g. Deluxe Sea View" className="admin-input" />
                </Field>
                <Field label="Size">
                  <input name="size" value={form.size} onChange={handleChange} placeholder="e.g. 45m²" className="admin-input" />
                </Field>
                ...
                <Field label="Price per Night (AED) *">
                  <input name="price" type="number" min="0" value={form.price} onChange={handleChange} required placeholder="550" className="admin-input" />
                </Field>
              </div>
            ...
            <Section title="Room Images">
              <div
                ...
                className="border-2 border-dashed border-outline-variant rounded-lg p-8 text-center cursor-pointer hover:border-primary-container hover:bg-surface-container-low transition group"
              >
                <span className="material-symbols-outlined text-3xl text-on-surface-variant group-hover:text-primary-container mx-auto mb-2 block transition">
                  add_photo_alternate
                </span>
                <p className="text-sm text-on-surface-variant group-hover:text-primary-container transition">Click to upload room photos</p>
                ...
                <div className="grid grid-cols-3 gap-3 mt-3">
                  {previews.map((p, idx) => (
                    <div key={idx} className="relative group rounded-lg overflow-hidden aspect-video bg-surface-container-high">
                      ...
                      {idx === 0 && (
                        <span className="absolute bottom-1.5 left-1.5 text-[10px] bg-secondary-container text-primary px-1.5 py-0.5 rounded font-medium">
                          Cover
                        </span>
                      )}
                    </div>
                  ...
                  <div
                    onClick={() => fileRef.current?.click()}
                    className="aspect-video border-2 border-dashed border-outline-variant rounded-lg flex items-center justify-center cursor-pointer hover:border-primary-container transition"
                  >
                ...
                {ROOM_AMENITIES.map((a) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-sm border transition ${
                      form.amenities.includes(a) ? "bg-primary-container text-white border-primary-container" : "border-outline-variant text-on-surface-variant hover:border-primary-container"
                    }`}
                  >
                    {a}
                  </button>
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                disabled={saving}
                className="flex-1 bg-primary-container text-white py-3 rounded-lg font-label-bold text-label-bold hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2"
              >
