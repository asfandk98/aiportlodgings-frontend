      ...
      <div className="flex-1 overflow-y-auto p-lg">
        <div className="space-y-md max-w-3xl">
          <div className="flex justify-end">
            <button onClick={openCreate} className="flex items-center gap-2 bg-primary-container text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition">
          ...
          {showForm && (
            <div className="bg-white border border-secondary-container rounded-xl p-lg space-y-md">
              ...
              <div className="space-y-1.5">
                ...
                <input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. Layover Guides" className="admin-input" />
              </div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className={`w-10 h-5 rounded-full transition-colors relative ${form.status === "active" ? "bg-primary-container" : "bg-surface-container-high"}`}
                >
              ...
              <button onClick={save} disabled={saving} className="flex items-center gap-2 bg-primary-container text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-50">
          ...
          {loading ? (
            <div className="flex items-center justify-center h-40">
              <div className="w-6 h-6 border-4 border-secondary-container border-t-transparent rounded-full animate-spin" />
            </div>
          ) : (
              ...
              <table className="w-full text-sm zebra-table">
                <thead>
                  <tr className="border-b border-outline-variant text-on-surface-variant text-xs uppercase tracking-wider">
                    <th className="text-left px-lg py-4">Name</th>
                    <th className="text-left px-lg py-4">Description</th>
                    <th className="text-left px-lg py-4">Posts</th>
                    <th className="text-left px-lg py-4">Status</th>
                    <th className="text-right px-lg py-4">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {categories.length === 0 ? (
                    ...
                    categories.map((cat) => (
                      <tr key={cat.id}>
                        <td className="px-lg py-4 font-medium text-primary">{cat.name}</td>
                        <td className="px-lg py-4 text-on-surface-variant max-w-xs truncate">{cat.description ?? "—"}</td>
                        <td className="px-lg py-4 text-on-surface-variant">{cat.posts_count ?? 0}</td>
                        ...
                        <td className="px-lg py-4">
                          <div className="flex items-center justify-end gap-2">
                            <button onClick={() => openEdit(cat)} className="text-primary-container hover:text-secondary transition">
                              <span className="material-symbols-outlined text-lg">edit</span>
                            </button>
                            <button onClick={() => remove(cat.id)} className="text-primary-container hover:text-error transition">
