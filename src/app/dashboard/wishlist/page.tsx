    ...
    <div className="space-y-md">
      {hotels.length === 0 ? (
        <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-xl text-center">
          <span className="material-symbols-outlined text-4xl text-on-surface-variant/40 mb-2 block">favorite</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-md">
          {hotels.map((h) => {
            ...
            return (
              <div key={hotelId} className="relative bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden group">
                <Link href={`/hotels/${h.slug}`} className="block">
                  ...
                  <div className="p-md">
                    <p className="font-headline-md text-[16px] text-primary truncate">{h.name ?? h.title}</p>
                    <p className="text-primary-container text-sm mt-1">AED {Number(h.price ?? 0).toLocaleString()}/night</p>
                  </div>
                </Link>
