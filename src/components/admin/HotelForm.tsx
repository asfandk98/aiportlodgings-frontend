...
const AMENITY_OPTIONS = [
  ...
  "Business Center", "Laundry", "24h Check-in", "Luggage Storage", "Soundproofing",
];
const HOTEL_TYPES = ["Luxury", "Resort", "Boutique", "Business", "Budget", "Transit"];
...
export default function HotelForm({ hotelId }: { hotelId?: string }) {
  ...
  const [form, setForm] = useState<HotelFormState>({
    title: "", location: "", price: "", type: "Transit",
  ...
  return (
    <div className="max-w-4xl space-y-md pb-xl">
      <form onSubmit={handleSubmit} className="space-y-md">
        <Section title="Basic Information">
          <div className="grid grid-cols-2 gap-md">
            <Field label="Title *">
              <input name="title" value={form.title} onChange={handleChange} required className="admin-input" placeholder="e.g. Dubai Transit Hotel & Suites" />
            </Field>
            <Field label="Location *">
              <input name="location" value={form.location} onChange={handleChange} required className="admin-input" placeholder="e.g. Terminal 3, DXB" />
            </Field>
            ...
            <Field label="Star Rating">
              <input name="rating" type="number" step="0.1" min="1" max="5" value={form.rating} onChange={handleChange} className="admin-input" placeholder="4.0" />
            </Field>
          </div>
        </Section>
        ...
        <Section title="Main Image">
          <label className="block cursor-pointer">
            <div className="border-2 border-dashed border-outline-variant hover:border-primary-container rounded-lg p-8 text-center transition">
              {imagePreview ? (
        ...
        <div className="flex gap-3">
          <button
            type="submit"
            disabled={saving}
            className="flex-1 bg-primary-container text-white py-3 rounded-lg font-label-bold text-label-bold hover:opacity-90 transition disabled:opacity-50"
          >
...
function Toggle({ label, description, active, onToggle }: { label: string; description: string; active: boolean; onToggle: () => void }) {
  return (
    <div className="flex items-center gap-3 cursor-pointer" onClick={onToggle}>
      <div className={`w-10 h-5 rounded-full transition-colors relative ${active ? "bg-primary-container" : "bg-surface-container-high"}`}>
